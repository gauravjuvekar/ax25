#include "common_functions.h"

static const uint8_t byte_rev_table[] = {
	
};

uint8_t byterev(uint8_t byte) {
/* Returns a reflected byte
 */
	return byte_rev_table[byte];
}


bool get_bit(const uint8_t *src, size_t src_n, struct get_bit_state *state) {
/* Returns the next single bit from the source.
 * state->src_consumed is set to true if the source is consumed.
 * The correct bit is still returned
 */
	bool bit = (src[state->src_index] & state->src_mask)?true:false;
	state->src_mask >>= 1;
	if(state->src_mask == 0) {
		state->src_mask = UINT8_T_MSB_MASK;
		state->src_index += 1;
		if(!(state->src_index < src_n)) {
			state->src_consumed = true;
		}
	}
	return bit;
}


void set_bit(uint8_t *dest, size_t dest_n,
		bool bit, struct set_bit_state *state) {
/* Sets the destination bit.
 * state->dest_filled is set to true if the destination is filled
 * after setting the bit
 */
	if(bit) {
		dest[state->dest_index] |= state->dest_mask;
	}
	else {
		dest[state->dest_index] &= ~state->dest_mask;
	}
	state->dest_mask >>= 1;
	if(state->dest_mask == 0) {
		state->dest_mask = UINT8_T_MSB_MASK;
		state->dest_index += 1;
		if(!(state->dest_index < dest_n)) {
			state->dest_filled = true;
		}
	}
}
